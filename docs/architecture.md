...
## Debug Render Site
A static debug site generator lives in `src/scripts/generateDebugSite.ts`. It spins up five seeded runs of the terrain and hydrology pipeline, serialises the rasters and polylines into JSON and emits an `index.html` that draws elevation, moisture and river overlays onto canvases for quick inspection. The `npm run generate:debug-site -- --runs 5 --out debug-site` script executes the generator via `ts-node`, writes the site folder and is executed by `.github/workflows/debug-renders.yml`, which uploads the result as a GitHub Pages artifact on pushes to `main` or manual dispatch.
